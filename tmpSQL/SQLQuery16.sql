...
DECLARE @date date = '2020-08-01',@id_ObjectLease int = 1,@id_tPlanReport int = 0 
...
select f.id_Agreements,sum(f.Summa) as summa INTO #tmpFinesOver from Arenda.j_Fines f inner join Arenda.s_AddPayment a on a.id = f.id_ÀddPayment where f.PlanDate = @date and a.cName not like 'Ïåíè' group by f.id_Agreements
...
where 
	a.isConfirmed = 1 and a.Start_Date<= @date and @date<=a.Stop_Date and td.Rus_Name = 'Àêò ïðè¸ìà-ïåðåäà÷è' and a.id_ObjectLease = @id_ObjectLease
