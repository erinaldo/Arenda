...
namespace GdiPlusLib
{
    ...
    public class Gdip
    {
        ...
        public static bool SaveDIBAsFile(string picname, IntPtr bminfo, IntPtr pixdat)
        {
            string FileName = picname.Trim();
            //        MessageBox.Show(FileName);
            Guid clsid;
            if (!GetCodecClsid(FileName, out clsid))
            {
                MessageBox.Show("Неопределенный тип образа " + Path.GetExtension(FileName),
                                "Тип образа", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
