...
namespace Arenda
{
    public partial class frmAddEditBank : Form
    {
        ...
        private void Save()
        {
            ...
            {
                DataTable dtResult = proc.addeditBank(id_bank, tbName.Text, tbCA.Text, tbBIC.Text,
                    id_bank == 0 ? 1 : 0, 1);

                Logging.StartFirstLevel(id_bank == 0 ? 1385 : 1386);
                ...
                if (id_bank == 0)
                {
                    ...
                    Logging.Comment("Корр. счет банка: " + tbCA.Text);
                    Logging.Comment("БИК банка: " + tbBIC.Text);
                }
                else
                {
                    Logging.VariableChange("Наименование банка: " , tbName.Text, bankName);
                    Logging.VariableChange("Корр. счет банка: " , tbCA.Text, CorrAcc);
                    Logging.VariableChange("БИК банка: " , tbBIC.Text, BIC);
                }
            ...
            else
            {
                ...
                else if (id_bank == 0)
                {
                      ...
                      MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                    {
                        ...
                        Logging.Comment($"Смена статуса основания заключения договоров на действующий");
                        Logging.Comment($"ID:{dtBank.Rows[0]["id"]}");

                        Logging.Comment($"Наименование банка:  {dtBank.Rows[0]["cName"]}");
                        Logging.Comment($"Корр. счет банка:  {dtBank.Rows[0]["CorrespondentAccount"]}");
                        Logging.Comment($"БИК банка:  {dtBank.Rows[0]["BIC"]}");
