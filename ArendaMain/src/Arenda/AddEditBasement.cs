    ...
    public partial class AddEditBasement : Form
    {
        ...
        private void btAdd_Click(object sender, EventArgs e)
        {
            ...
            if (dtb == null || dtb.Rows.Count == 0)
            {
                if (_mode == 1)
                {
                    DataTable dtResult = _proc.AddEditBas(0, tbCname.Text, tbAbbr.Text, 1, 1, boom);
                ...
                if (_mode == 0)
                {
                    ...
                    Logging.VariableChange("Наименование основания заключения договоров: ", tbCname.Text.Trim(), _cName);
                    Logging.VariableChange("Аббревиатура основания заключения договоров: ", tbAbbr.Text.Trim(), _abbr);
            ...
            else
            {
                ...
                else if (zid == 0)
                {
                    if (MessageBox.Show("Введённое наименование присутствует в БД и имеет статус \"недействующее\". Вы хотите изменить статус на \"действующее\"?", "Сохранение записи", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                    {
                        ...
                        Logging.Comment($"ID:{dtb.Rows[0]["id"]}");
                        
                        Logging.Comment($"Наименование основания заключения договоров: {dtb.Rows[0]["cName"]}");
                        Logging.Comment($"Аббревиатура основания заключения договоров: {dtb.Rows[0]["Abbreviation"]}");
                        Logging.Comment($"Наличие номера и даты у договора: {((bool)dtb.Rows[0]["needDate"] ? "Да" : "Нет")}");
