...
-- Description:	—ÓÁ‰‡ÌËÂ ËÎË Û‰‡ÎÂÌËˇ Á‡„ÓÎÓ‚Í‡ ÂÊÂÏÂÒˇ˜ÌÓ„Ó ÔÎ‡Ì‡
-- =============================================
ALTER PROCEDURE [Arenda].[spg_setTMonthPlan]		 
	@id int,	
...
IF @id = 0
	BEGIN
		INSERT INTO [Arenda].[j_tMonthPlan]
			   ([PeriodMonthPlan]
			   ,[id_ObjectLease]
			   ,[is—onfirmed]
			   ,[id_—onfirmed]
			...
			BEGIN
				IF @result = 0
					BEGIN
						...
						if not exists (select top(1) id from Arenda.j_tMonthPlan where id = @id )
							BEGIN								
								select -1 as id
								return
							END

						if exists (select top(1) id from Arenda.j_tMonthPlan where id = @id and is—onfirmed = 1)
							BEGIN								
								select -2 as id
								return
							END
